...
void ssl_backward(SSL *serverssl, SSL *remote_ssl)
{
	...
	while ((recvlen = SSL_read(remote_ssl, buf, BUFFER)) > 0)
	{
		...
		//cout << "BackWard Thread" << endl;
		cout << buf << endl;
		EnterCriticalSection(&cs);
		if (SSL_write(serverssl, buf, recvlen) == SOCKET_ERROR)
		{
